...
var testARScene = createReactClass({
  ...
  _onSceneAnchorFound(anchor) {
    ...
    planes.push(
      <ViroARPlane
        ...
        <ViroSurface rotation={[-90, 0, 0]} scale={[0.5, 0.5, 1]} materials={'blue_plane'} />
      </ViroARPlane>,
    );
  ...
  _setTextNatively(callbackType, num) {
    return () => {
      ...
      this._callbackText.setNativeProps({
        text:
          'Callback: ' +
          'index' +
          num +
          '| ' +
          this.anchors[num].position[0] +
          ',' +
          this.anchors[num].position[1] +
          ',' +
          this.anchors[num].position[2],
      });
    };
  },
  ...
  _updateAnimation(position) {
    ViroAnimations.registerAnimations({
      moveToPos: {
        properties: { positionX: position[0], positionY: position[1] + 0.2, positionZ: position[2] },
        duration: 2000,
        delay: 0,
      },
    });
    this._animComponent.setNativeProps({ animation: { name: 'moveToPos', delay: 0, loop: false, run: true } });
  },
});
...
ViroAnimations.registerAnimations({
  ...
  sequentialAnim: [['moveRight', 'moveLeft']],
  parallelAnim: [['moveRight', 'moveLeft'], ['loopRotate']],
});
